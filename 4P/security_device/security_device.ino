...
// PIN setup
#define PIR_PIN 2      // PIR sensor signal connected to Pin 2
...
// Counter for 1 Hz PIR sampling at 10 Hz main loop rate
int pirSampleCounter = 0;
int lastPirValue = 0;
...
void loop() {
  // 1. Read PIR Sensor at 1 Hz (Every 10 loops of 100ms)
  if (pirSampleCounter % 10 == 0) {
    lastPirValue = digitalRead(PIR_PIN);
    pirSampleCounter = 0;
  }
  pirSampleCounter++;

  // 2. Read LSM6DS3 Accelerometer data at 10 Hz
  ...
  bool motionDetected = (lastPirValue == 1);
  ...
  // 6. 100ms delay to run accelerometer at 10 Hz
  delay(100);
}